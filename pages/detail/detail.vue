<template>
	<view class="detail">
		<view  class="fixbg" :style="{'background': 'url('+songDetail.al.picUrl+')'}">
		</view>
			...
			<scroll-view scroll-y="true" >
				...
				<view class="detail-lyric">
					<view class="detail-lyric-wrap" :style="{ transform : 'translateY('+ - (lyricIndex - 1) * 82 +'rpx)' }">
						<view class="detaili-lyric-item" :class="{active : lyricIndex == index}" v-for="(item,index) in songLyric" :key="index">
							{{item.lyric}}
						</view>
					</view>
				</view>
				<view class="detail-like">
					...
					<view class="detail-like-item" @tap="handleToSimi(item.id)" v-for="(item,index) in songSimi" :key="item.id">
						<view class="detail-like-img" >
							<image :src="item.album.picUrl" mode=""></image>
						</view>
						<view class="detail-like-song" v-if="item.album">
							<view>{{item.name}}</view>
							<view>
			...
			getMusic(songId){
				...
				Promise.all([songDetail(songId),songSimi(songId),songComment(songId),songLyric(songId),songUrl(songId)]).then(res=>{
					...
					if(res[3][1].data.code == '200'){
						...
						// console.log(res[3][1].data.lrc.lyric)
						let re = /\[([^\]]+)\]([^\[]+)/g;
						// console.log("123", lyric.match(re) )
						var result =[];
...
<style scoped>
	...
	.detail-play view{width: 230rpx;height: 360rpx;background: url(../../static/needle.png);position: absolute;left: 100rpx;right:0rpx; top: -200rpx;margin: auto;background-size: cover;}
	.detail-lyric {font-size: 32rpx;color: #6f6e70; line-height: 82rpx;height: 246rpx;text-align: center;overflow: hidden;}
	.detail-lyric-wrap{ transition: 0.5s;}
	.detaili-lyric-item{height: 82rpx;}
	.detaili-lyric-item.active{color: white;}
	...
	.detail-comment-text{white-space: pre-line;font-size: 28rpx;line-height: 40rpx;color: white;margin-top: 20rpx;border-bottom: 1px #e0e0e0 solid;padding-bottom: 40rpx;}
</style>
